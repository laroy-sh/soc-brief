...
- Entra.News (Merill Fernando, Microsoft Entra product team) — https://entra.news/ (weekly Entra newsletter). The homepage is JS-heavy, so read the RSS feed at https://entra.news/feed, which has dated items. It's good for Entra ID, Conditional Access, ID Protection and Global Secure Access changes. Cite the linked Microsoft primary source, not the newsletter.
- Jussi Metso — https://www.jussimetso.com/ (personal blog on SOC operations, Sentinel/Defender XDR and Entra ID hardening such as PAW, PIM, Conditional Access and the Enterprise Access Model; posts every week or two). It's WordPress, and the feed lives at https://www.jussimetso.com/index.php/feed/ (the usual /feed path returns 404). Use it for practitioner analysis and architecture context. Cite the post itself only for his own analysis, and the Microsoft primary source for any product change.
- Microsoft Security Sentinel (José Lázaro Pinos) — https://securitysentinel.substack.com/ (Substack newsletter on Microsoft security, mainly Sentinel and Defender XDR; posts are irregular, sometimes months apart). Read the RSS feed at https://securitysentinel.substack.com/feed. Use it for practitioner write-ups and cite the Microsoft primary source for any product change.
