...
Surfaces Group Policy password policy changes from Defender for Identity directory events, including which policy and which domain was touched. ([MDI-AD-GroupPolicy-PasswordPolicy — Alex Verboon](https://github.com/alexverboon/Hunting-Queries-Detection-Rules/blob/main/Defender%20For%20Identity/MDI-AD-GroupPolicy-PasswordPolicy.md))
...
Extracts new Tenant Allow Block List entries with their notes, expiry and defanged URL, so allow-list additions get reviewed rather than forgotten. ([New TenantAllowBlockList (TABL) entry — Jay Kerai](https://github.com/jkerai1/KQL-Queries/blob/main/Office%20365/New%20TenantAllowBlockList%20(TABL)%20entry.kql))
