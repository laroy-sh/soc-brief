...
Turns Automation Account job logs into per-runbook start, end and status rows, so a runbook that suddenly starts failing or running at odd hours is visible. ([AutomationAccount-RunbookStatus — Alex Verboon](https://github.com/alexverboon/Hunting-Queries-Detection-Rules/blob/main/AutomationAccounts/AutomationAccount-RunbookStatus.md))
...
Hunts the named pipe pattern behind the RedSun technique, which abuses a Defender detection flow to escalate to SYSTEM. ([Defender -RedSun Detection - NamedPipe Detection — Jay Kerai](https://github.com/jkerai1/KQL-Queries/blob/main/Defender/Defender%20-RedSun%20Detection%20-%20NamedPipe%20Detection.kql))
