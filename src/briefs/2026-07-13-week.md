...
Matches the CloudFront domain pattern used by several loader families where msiexec or an installer process is the parent. ([DeviceNetworkEvents-Suspicious process connection to cloudfront domain — Jose Sebastián Canós](https://github.com/ep3p/Sentinel_KQL/blob/main/Queries/Defender%20for%20Endpoint/DeviceNetworkEvents-Suspicious%20process%20connection%20to%20cloudfront%20domain.kql))
...
Extracts the exclusion changes from Conditional Access policy create and update events, so a quietly added exempt user or group does not go unnoticed. ([Microsoft Entra Conditional Access Policy Exclusion Modification — Benjamin Zulliger](https://github.com/benscha/KQLAdvancedHunting/blob/main/Audit/Microsoft%20Entra%20Conditional%20Access%20Policy%20Exclusion%20Modification.md))
