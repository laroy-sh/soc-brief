...
Joins network connections back to their initiating process to catch msiexec spawning a child that then calls out, the classic MSI-delivered loader pattern. ([DetectMsiexecExecutingDllNetworkConnections — Robbe Van den Daele](https://github.com/HybridBrothers/Hunting-Queries-Detection-Rules/blob/main/Defender%20For%20Endpoint/DetectMsiexecExecutingDllNetworkConnections.md))
...
Reads the RunMRU registry key for encoded PowerShell, curl and mshta invocations, which is where ClickFix-style paste-and-run lures leave their fingerprint. ([SuspiciousRUNMRUEntry — Bert-Jan Pals](https://github.com/Bert-JanP/Hunting-Queries-Detection-Rules/blob/main/Defender%20For%20Endpoint/SuspiciousRUNMRUEntry.md))
