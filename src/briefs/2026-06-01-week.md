...
Tracks changes to the Teams external access allowed-domains list, a federation setting that quietly widens who can message your users. ([Teams-AllowedDomains — Alex Verboon](https://github.com/alexverboon/Hunting-Queries-Detection-Rules/blob/main/Defender%20For%20Office%20365/Teams-AllowedDomains.md))
...
Filters NTLM network logons down to devices the exposure graph marks as critical, which is where an NTLM relay actually hurts. ([NTLM Network Logon to Critical Device — Benjamin Zulliger](https://github.com/benscha/KQLAdvancedHunting/blob/main/ThreatHunting/NTLM%20Network%20Logon%20to%20Critical%20Device.md))
